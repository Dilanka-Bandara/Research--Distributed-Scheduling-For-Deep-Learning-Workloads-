...
V09 FAIRNESS FIX — profiling on the fast path
  V08's fast path reserved with stall_rounds=0.0 even on a historical-cache
  MISS, so a model SMART had never seen ran at full speed immediately, while
  FFT (and SMART's own slow path) paid the 60 s on-the-fly profiling stall.
  That handed SMART free knowledge of theta and is where the "40x fewer
  profiling stalls" figure came from. A cache miss now pays the same stall on
  either path. The cache itself (component 3) is a legitimate SMART feature:
  a model profiled once in a run is not profiled again — so SMART should show
  at most one stall per distinct model, not zero.
"""
...
def run():
    ...
    while r.get("shutdown") != "1":
        ...
        # Profiling cost is paid on a cache miss regardless of route.
        ...
        if go_fast:
            ...
            for th, fr, free, target in candidates:
                ...
                ack = rpc(r, target, "reserve",
                          {"job": jid, "stall_rounds": stall, "route": "fast"})
                if ack.get("ok"):
                    update_job(r, jid, route="fast")
                    if stall:
                        emit(r, "profile_stall", job=jid, rounds=stall)
