...
class ArrivalPlan:
    ...
    phases: List[Phase]
    shock_onset: Optional[float] = None   # rounds; start of the PRIMARY window
    shock_end: Optional[float] = None     # rounds; end of the PRIMARY window
    # Every stressed stretch, not just the primary one. dyn_flash has THREE
    # impulses; labelling only the middle one as "shock" silently files the
    # other two under "recovery" and corrupts the baseline-vs-shock comparison.
    shock_windows: List[tuple] = field(default_factory=list)
    ...
    def windows(self) -> List[tuple]:
        if self.shock_windows:
            return self.shock_windows
        if self.shock_onset is None:
            return []
        return [(self.shock_onset, self.shock_end)]

    # A job arriving within this many rounds after a stress window closes is
    # "recovery": it lands on a cluster still digesting the surge. ~One median
    # job duration (calibrate_load.py reports ~10-12 rounds).
    RECOVERY_ROUNDS = 10.0

    def cohort(self, t: float) -> str:
        """Which experimental cohort a job arriving at `t` belongs to.

        shock     arrived inside ANY stress window
        recovery  arrived in calm, within RECOVERY_ROUNDS after a window closed
        baseline  every other calm arrival — the scheduler's own calm behaviour

        V09 NOTE. Two earlier definitions were both wrong for multi-window
        regimes. The original used only the MIDDLE window, so dyn_flash's first
        impulse counted as baseline and its third as recovery — surge jobs in
        the denominator. The interim fix counted all windows as shock but made
        "baseline" only the jobs before the first window (8 of 40), a
        denominator too thin to mean anything. Baseline is now all calm
        arrivals not in a recovery tail, which is both uncontaminated and large.
        """
        wins = self.windows()
        if not wins:
            return "baseline"
...
def build_plan(regime: str, seed: int, horizon: float) -> ArrivalPlan:
    ...
    if regime == "dyn_sine":
        ...
        wins = [(p.t0, p.t1) for p in phases if p.label == "high"]
    ...
    if regime == "dyn_flash":
        ...
        wins = [(max(0.0, c - 2 * width), min(horizon, c + 2 * width))
                for c in centres]
        c = centres[1]
        return _finalise(
            ArrivalPlan(
