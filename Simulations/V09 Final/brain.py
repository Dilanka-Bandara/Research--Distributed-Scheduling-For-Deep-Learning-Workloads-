...
V09 FIXES
---------
1. THE 58-SECOND FREEZE. _corrective_migrations iterated a `jobs` snapshot
   taken BEFORE _place_queued_jobs ran. A job that was queued in that snapshot
   (gpu=None) and had just been placed was no longer in queued_ids, so it was
   treated as a running job to migrate — with src=None. The brain then sent
   `evict` to agent:None:req, which no agent serves, and blocked. With redis-py
   8's retries that block lasted ~58 s (19.5 rounds). The brain froze three
   times in a 40-job dyn_flash run, i.e. it was dead ~40% of the run: no ILP,
   no placements, no Mechanism E, so every slow-path job arriving in those
   windows waited ~19.5 rounds. That — not the architecture — is the
   "recovery-cohort starvation". Each migration candidate is now re-read from
   Redis and skipped unless it is genuinely running somewhere.

2. NO MORE SLEEP-AND-HOPE EVICTION. Coordinated preemptive placement and
   Mechanism E now send one atomic reserve carrying the victims
   (node_agent handles evict-then-reserve inside a single request), and
   corrective migration relies on the now-synchronous evict. The three
   time.sleep(0.15) calls are gone.
"""
    ...
    for j in queued_jobs:
        ...
        if free_t >= j["d"]:
            ...
            if running_victims and (free_t + running_victims[0][2] >= j["d"]):
                ...
                stall = j.get("pending_stall", 0.0) or 0.0
                # one atomic request: agent evicts the victim, then reserves
                ack = rpc(r, target, "reserve",
                          {"job": jid, "stall_rounds": stall,
                           "migrate_from": j.get("gpu"), "evict": [victim_id]})
                if ack.get("ok"):
                    r.lrem("queue:global", 0, str(jid))
                    update_job(r, jid, pending_stall=0.0)
...
def _corrective_migrations(r, jobs, alloc, queued_ids):
    ...
    for snap in jobs:
        ...
        if jid in queued_ids:
            continue
        # V09: re-read. The snapshot predates this wake's placements, so a job
        # that was queued in it (gpu=None) may have just been placed.
        j = load_job(r, jid)
        if not j or j.get("status") != "running" or not j.get("gpu"):
            continue
        target = alloc.get(jid)
        if target is None or j["gpu"] == target: continue
        src = j["gpu"]
        free_t = int(r.get(f"free:{target}") or 0)
        if free_t < j["d"]:
            continue
        if rpc(r, src, "evict", {"job": jid}).get("ok"):   # synchronous now
            ack = rpc(r, target, "reserve",
...
def _mechanism_e(r, t_rounds):
    ...
    for jid_s in list(r.lrange("queue:global", 0, -1)):
        ...
        for _, g in feas:
            ...
            stall = j.get("pending_stall", 0.0) or 0.0
            # V09: one atomic request — the agent evicts `picked` and reserves
            # in the same handler, so no fast-path arrival can take the slots.
            ack = rpc(r, g, "reserve",
                      {"job": j["id"], "stall_rounds": stall, "evict": picked})
            if ack.get("ok"):
