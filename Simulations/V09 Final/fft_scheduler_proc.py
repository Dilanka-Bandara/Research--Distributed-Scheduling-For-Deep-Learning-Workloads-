...
V09 FIXES (both in FFT's favour — worth saying so in the thesis)
  1. DRIFT-FREE ROUND GATE. V08 slept one full round AFTER finishing the
     previous round's solve and RPCs, so every round lasted 1 round + its own
     processing time and the schedule drifted later and later. That penalised
     FFT, and penalised it MORE on two machines where RPCs are slower. Rounds
     now fire on fixed boundaries t0 + k*round, as in the paper.
  2. Migration relies on the agent's now-synchronous evict; the
     time.sleep(0.15) race (shared with the brain) is gone.
  The ILP (ilp_core.py) is untouched.
"""
...
def run():
    ...
    known: dict[int, bool] = {}
    emit(r, "fft_up")
    round_real = rounds_to_real(1.0)
    gate0, k = time.monotonic(), 0
    while r.get("shutdown") != "1":
        k += 1                                       # the round gate:
        wait = gate0 + k * round_real - time.monotonic()   # fixed boundaries,
        if wait > 0:                                  # no cumulative drift
            time.sleep(wait)
        else:
            k += int(-wait // round_real)            # overran: skip, don't pile up
        ...
        for j in jobs:
            ...
            if j.get("gpu") is None:
                ...
                if int(r.get(f"free:{target}") or 0) < j["d"]:
                    continue
                if rpc(r, src, "evict", {"job": jid}).get("ok"):   # synchronous
                    ack = rpc(r, target, "reserve",
