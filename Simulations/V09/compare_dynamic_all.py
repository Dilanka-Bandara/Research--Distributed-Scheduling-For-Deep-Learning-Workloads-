...
METRICS = [
    ("dyn_excess_ttfe_rounds", "lower", "excess wait, shock vs calm"),
    ("dyn_excess_ttfe_p95_rounds", "lower", "excess wait p95"),
    ("dyn_excess_jct_rounds", "lower", "excess JCT, shock vs calm"),
    ("dyn_shock_ttfe_rounds", "lower", "shock-cohort wait (absolute)"),
    ("dyn_shock_jct_rounds", "lower", "shock-cohort JCT (absolute)"),
    ("dyn_reaction_rounds", "lower", "reaction to surge"),
    ("dyn_backlog_peak", "lower", "peak backlog (jobs)"),
    ("dyn_backlog_overshoot", "lower", "backlog overshoot"),
    ("dyn_backlog_auc_shock_jobrounds", "lower", "backlog area during surge"),
    ("dyn_settling_rounds", "lower", "settling time after surge"),
    ("dyn_stranded_gpu_rounds", "lower", "stranded GPU-rounds"),
    ("dyn_ttfe_p99_rounds", "lower", "wait p99 (whole run)"),
    # Context, not adaptability — included so the honest framing is visible.
    ("jct_mean_rounds", "lower", "  [context] run-wide mean JCT"),
    ("decision_latency_rounds_mean", "lower", "  [context] admission latency"),
]
...
def report_regime(regime: str, runs: Dict[str, Dict[int, dict]], verbose: bool):
    ...
    complete = all(
        r[s].get("n_finished", 0) >= (r[s].get("n_jobs") or 0) > 0
        for r in (fft, smart) for s in seeds
    )
    ...
    if marker == "NOT QUOTABLE":
        print("    (needs 3 paired seeds with every job finished)")
    print(f"{'=' * 92}")
