...
def generate_trace(
    ...
    regime:
      STATIONARY (constant arrival rate — these three are unchanged and still
      produce byte-identical traces, so all previously collected results
      remain valid):
        - "steady": near-uniform arrivals, smaller jobs dominate.
        - "mixed":  Poisson arrivals, balanced model mix.
        - "bursty": arrivals clustered into bursts. NOTE: bursty clusters
                    arrivals around random centres but the RATE is still
                    stationary in expectation — it is variance, not
                    non-stationarity.

      DYNAMIC (lambda(t) genuinely varies during the run — see
      arrival_process.py):
        - "dyn_step", "dyn_sine", "dyn_mmpp", "dyn_flash"

    Any OTHER name silently produced a "mixed" trace. That is how the regimes
    "dynamic", "heavy" and "random" in run_all_36.py came to be byte-identical
    copies of "mixed" for the same seed. Unrecognised names now warn loudly.
    """
    from arrival_process import DYNAMIC_REGIMES    # lazy: arrival_process
    if regime in DYNAMIC_REGIMES:                  # imports Job from here
        from arrival_process import generate_dynamic_trace
        return generate_dynamic_trace(n_jobs, regime, seed, horizon)

    if regime not in ("steady", "mixed", "bursty"):
        import sys as _sys
        print(f"[workload] WARNING: unknown regime {regime!r} — falling back "
              f"to a stationary 'mixed' trace. This is almost certainly not "
              f"what you wanted; see arrival_process.DYNAMIC_REGIMES.",
              file=_sys.stderr)
