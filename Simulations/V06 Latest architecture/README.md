...
| **Mechanism E (new)** | **Starvation-triggered preemption**: a queued job that has never executed and has waited ≥ `starve_after` rounds may evict running fast-path jobs (longest-remaining first). Restores the FFT paper's Theorem-4.2 property (every job executes within bounded time of arrival), which non-preemptible fast-path locks had broken. Livelock-free: victims have already executed, so they can never trigger it back. | `run_brain` preemption pass; dials `preempt_starved`, `starve_after`, `max_evict_per_round`; metric `preemptions` |
...
- Bursty *without* preemption: JCT ×1.09–1.14, FTF ×1.22–1.37, starvation ~×3 —
  the known worst case.
- **Optimised defaults (jct_aware_fast=True, starve_after=1.0): bursty starvation
  ×0.28, FTF ×0.92; steady starvation ×1.12 (near parity, from ×1.67); mixed JCT
  ×1.01 (parity). jct_aware_fast ablation: mixed JCT ×1.07 → ×1.01, FTF ×1.00 →
  ×0.91 — within-threshold fast placements pick the fastest-finishing type
  (FFT Eq.-3-consistent) instead of the ΔS-closest.**
- Bursty with Mechanism E at starve_after=3.0: starvation ×0.47 (better than FFT),
  JCT ×1.06, FTF ×0.96, latency speedup unchanged, ~78 preemptions.** Mixed:
  starvation ×4.09 → ×0.75. Steady: ×3.01 → ×1.60. All jobs finish; the
  mechanism is livelock-free by construction.
