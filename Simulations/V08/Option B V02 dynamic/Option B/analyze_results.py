...
def metrics(ev, jobs):
    ...
    # To fix cross-machine clock skew (which causes negative starvation),
    # compute first execution time entirely in PC 1 time (placed event + stall)
    first_placed = {}
    for e in ev:
        if e["type"] == "placed" and e["job"] not in first_placed:
            first_placed[e["job"]] = e["ts"]

    starv = []
    for j in fin:
        jid = j["id"]
        if jid in first_placed:
            # Reconstruct the stall applied on the first placement
            stall = 0.0
            # Actually, `node_agent.py` applies `stall_rounds` passed in the RPC.
            # For FFT, it's always PROFILE. For SMART, it's PROFILE if not cache_hit.
            if j.get("route") == "fft":
                stall = 60.0 / 300.0 # PROFILE_SECONDS / ROUND_SECONDS
            elif not j.get("cache_hit"):
                stall = 60.0 / 300.0

            from config import rounds_to_real
            pc1_first_exec = first_placed[jid] + rounds_to_real(stall)
            starv.append(real_to_rounds(pc1_first_exec - j["arrival_ts"]))
